...
//
// Fields:
// - Params: A slice of FilterParam representing the filter conditions to be
...
// This method allows differentiating ORParam from other types of query
// parameters.
func (p ORParam) ParamType() string {
	return TypeOR
}

// OR creates a new ORParam, which is a logical OR combination of the provided
// filter parameters.
//
// This function is used to build queries where you want to match records that
// satisfy any one of the given filter
// conditions.
//
// Parameters:
// - params: A variable number of Param, each of which should be a FilterParam.
//
// Returns: An ORParam that encapsulates the provided filter parameters in an OR
// logic.
//
// Example:
...
//	)
//
// This example creates query parameters that match records where 'id' is either
// 1 or 2.
//
...
func OR(params ...Param) Param {
	...
	for _, p := range params {
		...
		if !ok {
			panic(
				fmt.Errorf(
					"OR only accept FilterParam but got %s",
					p.ParamType(),
				),
			)
		}
